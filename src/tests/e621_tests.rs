...
use crate::caption::{process_e621_json_data, process_e621_tags, should_ignore_e621_tag, E621Config};
...
fn test_artist_formatting() {
    ...
    let config = E621Config::new()
        .with_artist_prefix(Some("drawn by ".to_string()));
...
async fn test_process_real_e621_test_files() -> anyhow::Result<()> {
    ...
    for i in 1..=3 {
        ...
        if let Some(post) = json_data.get("post") {
            ...
            assert!(post.get("file").is_some(), "e621-{}.json missing file field", i);
            assert!(post.get("tags").is_some(), "e621-{}.json missing tags field", i);
            assert!(post.get("rating").is_some(), "e621-{}.json missing rating field", i);
            
            // Get the filename from the URL
            if let Some(url) = post["file"]["url"].as_str() {
                let filename = url.split('/').last().unwrap_or_default();
                let caption_path = PathBuf::from("../test_data").join(format!("{}.txt", filename.split('.').next().unwrap_or_default()));
                
                // Verify the caption file was created
                assert!(caption_path.exists(), "Caption file not created for e621-{}.json", i);
                ...
                assert!(caption_content.starts_with(&format!("{}, ", rating_text)), 
                    "Caption doesn't start with correct rating for e621-{}.json", i);
                
                // Verify artist tags are included
                if let Some(artists) = post["tags"]["artist"].as_array() {
                    for artist in artists {
                        if let Some(artist_name) = artist.as_str() {
                            let formatted_artist = format!("by {}", artist_name.replace('_', " ").replace(" (artist)", ""));
                            assert!(caption_content.contains(&formatted_artist), 
                                "Artist '{}' not found in caption for e621-{}.json", artist_name, i);
                        }
                    }
                }
...
async fn test_e621_config() -> anyhow::Result<()> {
    ...
    let config = E621Config::new()
        .with_rating_conversions(Some(custom_ratings));
